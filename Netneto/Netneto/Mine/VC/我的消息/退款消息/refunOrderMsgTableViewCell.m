...
@interface refunOrderMsgTableViewCell ()<UITextViewDelegate>
...
@end
@implementation refunOrderMsgTableViewCell
-(void)setModel:(ImRefundMsgInfo *)model{
    ...
    self.shopTitleInfoLabel.text = [NSString stringWithFormat:@"%@",model.prodName];
    ...
    if (model.refundType == 1) {
        ...
        self.addLabel.text = TransOutput(@"商家地址信息");
        self.addNameLabel.text = model.shopName;
        self.phoneLabel.text = model.tel;
        self.codeLabel.text = [NSString stringWithFormat:@"〒%@",model.returnPostalCode];
        NSString *str = [NSString stringWithFormat:@"%@-%@-%@  %@",model.returnProvince,model.returnCity,model.returnArea,model.returnAddress];
        self.addInfoLabel.text = str;
        [self.copBtn setTitle:TransOutput(@"复制") forState:UIControlStateNormal];
        @weakify(self);
        [self.copBtn addTapAction:^(UIView * _Nonnull view) {
            @strongify(self);
            UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
            [pasteboard setString:[NSString stringWithFormat:@"%@-%@-%@  %@",self.model.returnProvince,self.model.returnCity,self.model.returnArea,self.model.returnAddress]];
