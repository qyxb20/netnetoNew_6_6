...
- (void)_playItemWasTapped {
    ...
    if ( [self.delegate respondsToSelector:@selector(playItemWasTappedForControlLayer:)] ) {
        [self.delegate playItemWasTappedForControlLayer:_videoPlayer.isPaused];
    }
...
- (void)videoPlayerPlaybackStatusDidChange:(__kindof SJBaseVideoPlayer *)videoPlayer {
    ...
    if (videoPlayer.isPlaybackFinished) {
        [self.delegate playFinshForControlLayer:videoPlayer.isPlaybackFinished];
    }
    else{
        [self.delegate playFinshForControlLayer:NO];
    }
}
