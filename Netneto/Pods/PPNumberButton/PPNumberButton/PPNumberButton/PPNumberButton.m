...
#define ALPHANUM @"0123456789"//限制只能输入数字
...
{
//    if ([textField.text intValue] > self.stockValue) {
//        self.textField.text =[NSString stringWithFormat:@"%ld",(long)self.stockValue];
//    }
    if (!self.isCar) {
            if ([textField.text intValue] > self.stockValue) {
                self.textField.text =[NSString stringWithFormat:@"%ld",(long)self.stockValue];
    ...
    [self CustomClickCallBackWithIncreaseStatus:NO];
}
...
//            _tipBlock ? _tipBlock(@"已超过最大库存") : nil;
            if (self.limValue != 0 && self.stockValue > self.limValue) {
                _tipBlock ? _tipBlock(@"已超过单次最大购买数量") : nil;
            }else{
                _tipBlock ? _tipBlock(@"已超过最大库存") : nil;
            }
        }
...
//    [self checkTextFieldNumberWithUpdate];
    ...
    if (number >= _minValue) {
        ...
        _tipBlock ? _tipBlock(@"数量不能小于1") : nil;
...
- (void)CustomClickCallBackWithIncreaseStatus:(BOOL)increaseStatus
{
    
    _customBlock ? _customBlock(self,_textField.text.floatValue, increaseStatus) : nil;
    if ([_delegate respondsToSelector:@selector(pp_numberButton: number: increaseStatus:)]) {
        float f =[self.textField.text floatValue];
        if (!self.isCar) {
            self.textField.text = [NSString stringWithFormat:@"%d",(int)(f+0.5)];
            
        }
        
        [_delegate pp_CustomNumberButton:self number:(int)(f+0.5) increaseStatus:increaseStatus];
...
-(void)setStockValue:(NSInteger)stockValue{
    _stockValue = stockValue;
}

-(void)setLimValue:(NSInteger)limValue{
    _limValue = limValue;
}
